...
A = np.vstack([A, numb])
A_tr = A.transpose()
A_tr_sort = sorted(A_tr, key=lambda a_entry: a_entry[0], reverse = True) 
...
for i in range(n):
    if AA_tr_sort[i][1] <= C_ost:
        ...
        index.append(int(AA_tr_sort[i][2]))
        print ( int(AA_tr_sort[i][2]),": ", int(AA_tr_sort[i][0]), " ", int(AA_tr_sort[i][1]) )
    else: 
        tot_price += AA_tr_sort[i][0]
        tot_weight += AA_tr_sort[i][1]
        index.append(int(AA_tr_sort[i][2]))
...
tot_price = round(int(tot_price) - c2*(int(tot_weight) - int(CC)))
